...
(function () {
	...
	Completion.prototype.handleKeyup = function (event) {
		...
		if (this._hasInput && this._state === "VOID") {
			...
			// ============================================
			// JANE OPTIMIZATION FIX: ROBUST TRIGGER
			// ============================================
			// Instead of relying on _lastChar (which fails if you type faster than the event loop),
			// we simply check if the *current text* contains a valid, open pattern.
			// This catches "/t" even if the "/" keyup event was missed or overwritten.

			var idT;
			for (idT = 0; idT < this._listTemp.length; idT++) {
				// Check if we are currently inside a valid pattern context
				var potentialMatch = extractPattern(val, curPos, this._listTemp[idT]);
			...
			if (key === 13) { // ENTER
				...
				if (selected) {
					...
					insertInto(this._areaNode,
						str,
						pattern.start, curPos, this._template, temp.title);
				...
				if (pattern.text.length > (this._minPatLength - 1)) {
					...
					// ============================================
					// JANE OPTIMIZATION FIX v2
					// ============================================
					var allOptions;
					var baseFilter = (this._template) ? this._template.filter : "[all[tiddlers]!is[system]!is[shadow]]";

					// FIX: Always search the TITLE.
					var fieldMode = "title";

					// Clean up the input
					var cleanPat = pattern.text.replace(/\[/g, "").replace(/\]/g, "");

					// We append a search + limit to the filter. 
					var searchPart = " +[search:" + fieldMode + "[" + cleanPat + "]limit[" + (this._maxMatch + 5) + "]]";

					allOptions = this._widget.wiki.filterTiddlers(baseFilter + searchPart);
					// ============================================
					// JANE OPTIMIZATION END
					// ============================================
	 ...
	 */
	var insertInto = function (node, text, posBefore, posAfter, template, tiddlerTitle) {
		...
		// Determine offset: check caret-pos field first, then template.startOffset
		var offset = undefined;
		if (tiddlerTitle) {
			var tiddler = $tw.wiki.getTiddler(tiddlerTitle);
			if (tiddler && tiddler.fields["caret-pos"]) {
				var val = parseInt(tiddler.fields["caret-pos"], 10);
				if (!isNaN(val)) {
					offset = val;
				}
			}
		...
		if (offset === undefined && template.startOffset !== undefined) {
			offset = template.startOffset;
		}

		if (offset !== undefined) {
			let cursorPos = posBefore + offset;
